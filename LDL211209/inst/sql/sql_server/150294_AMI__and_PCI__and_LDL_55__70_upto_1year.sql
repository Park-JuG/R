...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4296653,312327,434376,438170,4270024,438172)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4296653,312327,434376,438170,4270024,438172)
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (42340539,42365791,42362201,42356898,42356820,45889355,43527998,4006788,4283892)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (42340539,42365791,42362201,42356898,42356820,45889355,43527998,4006788,4283892)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (42088744,42093315,42096799,42097413,42101481,42103370,42103371,42103372,42103373,42103374)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (42088744,42093315,42096799,42097413,42101481,42103370,42103371,42103372,42103373,42103374)
  and c.invalid_reason is null
...
-- End Condition Era Criteria
  ...
  WHERE DATEADD(day,0,OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,14,E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
) P
WHERE P.ordinal = 1
-- End Primary Events
...
JOIN #Codesets codesets on ((de.device_concept_id = codesets.concept_id and codesets.codeset_id = 3))
) C
...
WHERE (C.value_as_number >= 55.0000 and C.value_as_number <= 70.0000)
-- End Measurement Criteria

) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,14,P.START_DATE) AND A.START_DATE <= DATEADD(day,365,P.START_DATE)
GROUP BY p.person_id, p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) >= 1
-- End Correlated Criteria
...
WHERE C.value_as_number < 55.0000
-- End Measurement Criteria
